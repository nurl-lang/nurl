...
NURL has an official **online playground** where you can run NURL code directly in your browser. Same site provides HTTP APIs and an MCP server for integration with AI assistants.
...
- The hosted playground is **unauthenticated and public**-don't send secrets
...
The playground runs entirely in your browser-once compiled, WebAssembly is executed using a WASI shim with no server-side execution.
## HTTP API Endpoints
...
POST /build_macos
```
Cross-compiles via zig cc, linking only libSystem. Binaries are unsigned-clear the quarantine attribute before running.
...
The playground exposes a **public [Model Context Protocol](https://modelcontextprotocol.io/)** endpoint, enabling AI assistants to build and inspect NURL code.
...
You can compile the NURL compiler itself to WebAssembly, creating an offline, embeddable `nurlc.wasm`:
