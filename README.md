...
| **`^` is the `return` keyword, not XOR.** NURL has no infix XOR operator; `^ a b` always parses as `return (a b …)`. The compiler can't tell user-intent from syntax, so the LLVM-level error is "return expression has no value" or "value doesn't match function result type" | Implement bitwise XOR via the identity `a XOR b = (a OR b) - (a AND b)` — for u8: `@ __xor8 i a i b → i { ^ - ` `|` ` a b & a b }` |
