...
| P2c   | ≤ 400 ms   | Hoist 3 vec_reserve + 3 vec_data out of per-row loop | ✗ attempted 2026-05-16, no win post-LTO — runtime LTO (59e1207) already inlines `nurl_peek` / `nurl_poke` across the FFI boundary, collapsing the projected 90–180 ms savings to near zero. Pre-allocating flat_cells to nl_count × cells_est × 3 also costs ~30 ms of `memchr` newline scan + ~50 ms of page-fault on first-touch for the up-front 200–500 MB buffer. Net: ±10 ms vs. baseline 287 ms / 468 ms — within run-to-run noise. See `compare/HISTORY.md` 2026-05-16 entries for the side-by-side numbers. Reverted. The hoist itself works (modulo a mutable-`*i`-binding-with-many-uses miscompile that crashed at row ~66k; see investigation notes); the optimization just isn't load-bearing now that LTO is on. |
| SSE2 / C filter helpers | (partial) | SSE2 cell scan + C filter pipeline | ✓ shipped 2026-05-19 — load 313→278 ms (~11 %), filter 150→105 ms (~30 %), total 451→436 ms. `__csv_parse_content` inner cell loop now calls SSE2-vectorised `nurl_csv_scan_cell` (16 bytes/iter); filter goes through `csv_table_filter_float_gt` + `csv_table_filter_str_contains` (single FFI call per stage, inline `__csv_fast_atof`, first-byte-prefilter substring scan). Halving target NOT met — load is memory-bandwidth wall, filter is parse_float + scattered flat_cells access. Further drops likely require P3b (typed pre-parse during byte-walk) or P4 (columnar layout). |
| P3b (lite) | (infra)  | Typed pre-parse cache on CSVTable             | ✓ shipped 2026-05-19 — `csv_table_load_typed_f` + `csv_table_filter_typed_float_gt` route one column's parse through a load-time cache (`typed_floats` on CSVTable); regression `compiler/tests/csv_typed_float.nu`. NOT a win on `compare/test_data.csv`'s one-shot filter (combined-predicate helper already runs the float check at near-optimal speed). Useful for multi-pass / aggregate workloads where the parsed column is consumed more than once. |
| P3b   | ≤ 350 ms   | Schema-typed parse                            | ✗ standalone-extract attempt 2026-05-16 lost (475 → 525 ms median, +50 ms). `csv_table_extract_col_f64` / `_i64` shipped to `stdlib/ext/csv.nu` as a useful general-purpose feature (no harm in keeping), but on this bench the second full iteration to parse val_float2 (~130 ms) costs more than the per-row `nurl_parse_float_range` it saves in the filter (~90 ms). Real P3b win requires INTEGRATING typed parse into `__csv_parse_content`'s single byte-walk: bytes are already in L1 cache while the parser is reading them, so an inline `nurl_parse_float_range` per cell costs the parse function only — not a second pass over flat_cells/row_starts/row_lens. Estimated savings ~60 ms after integration. Bigger refactor: load signature grows a `Schema` parameter, parser branches per-cell on schema kind. Deferred — pick up when there is a real consumer (e.g. typed aggregation API for [project_csv_perf_post_lto](memory)'s P4 columnar workload). |
