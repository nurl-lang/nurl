...
// `prepare_ir_for_wasi`. nurlc's own prelude declares `puts` / `strcmp` /
// `fseek` with C-accurate i32 returns, while a program that FFI-declares
// a libc function itself gets NURL's widths: `& `libc` @ rand → i` emits
// `declare i64 @rand()`. One hardcoded shim shape cannot serve both — an
// i32 shim under an i64 call site (or the reverse) is a wasm signature
// mismatch, and wasm-ld answers those with a stub whose body is a single
// `unreachable`. The module links and reports BUILD OK, then traps on the
// first call: "runtime error: unreachable". doomfire / sand / starfield
// all died there on their first `( rand )`.
//
...
@ ir_decl_line String ir s name → String {
...
@ ir_decl_ret String line → String {
    : ~ String out ( string_new )
...
@ ir_decl_params String line → ( Vec String ) {
...
@ shim_ret_from_letters i r_char → String {
    ^ ( string_from ? == r_char 112 `i8*` ? == r_char 118 `void` ? == r_char 105 `i32` `i64` )
}

@ shim_pms_from_letters String pms → ( Vec String ) {
    : ( Vec String ) out ( vec_new [String] )
    : ~ i k 0
    ~ < k ( string_len pms ) {
...
@ libc_pm_ty String pms i k → s { ^ ? == ( string_get pms k ) 112 `i8*` `i32` }

// IR-side type of parameter k (what the call sites pass).
@ ir_pm_ty ( Vec String ) ir_pms i k → s {
    : ~ s out `i64`
...
@ shim_conv_kind s from s to → i {
    ? != 0 ( nurl_str_eq from to ) { ^ 0 } {}
    : b fp ( ir_ty_is_ptr from )
    : b tp ( ir_ty_is_ptr to )
    ? & fp tp { ^ 0 } {}
    ? fp { ^ 2 } {}
    ? tp { ^ 3 } {}
...
@ shim_conv_op i kind → s { ^ ? == kind 1 `trunc` ? == kind 2 `ptrtoint` ? == kind 3 `inttoptr` `sext` }

// `  %tK = <op> <from> %aK to <to>` — the argument-side conversion.
@ emit_shim_pm_conv String shims i kind s from s to i k → v {
    ? == kind 0 { ^ v } {}
    ( string_push_str shims `  %t` ) ( string_push_int shims k )
    ( string_push_str shims ` = ` ) ( string_push_str shims ( shim_conv_op kind ) )
...
@ emit_libc_shim String shims s name i r_char String pms String ir_ret ( Vec String ) ir_pms → v {
    ...
    ~ < k np {
        ...
        ( string_push_str shims ( libc_pm_ty pms k ) )
        = k + k 1
    }
    ( string_push_str shims `)\n` )
    ...
    ~ < k np {
        ? > k 0 { ( string_push_str shims `, ` ) } {}
        ( string_push_str shims ( ir_pm_ty ir_pms k ) )
        ( string_push_str shims ` %a` ) ( string_push_int shims k )
        = k + k 1
    }
    ( string_push_str shims `) {\n` )
    ...
    = k 0
    ~ < k np {
        ( emit_shim_pm_conv shims ( shim_conv_kind ( ir_pm_ty ir_pms k ) ( libc_pm_ty pms k ) )
        ( ir_pm_ty ir_pms k ) ( libc_pm_ty pms k ) k )
        = k + k 1
    }
    ...
    ~ < k np {
        ? > k 0 { ( string_push_str shims `, ` ) } {}
        ( string_push_str shims ( libc_pm_ty pms k ) )
        ( string_push_str shims ? == 0 ( shim_conv_kind ( ir_pm_ty ir_pms k ) ( libc_pm_ty pms k ) ) ` %a` ` %t` )
    ...
    ? ret_void { ( string_push_str shims `  ret void\n` ) } {
        ? tgt_void {
            ...
            : i rk ( shim_conv_kind tgt_ret ( string_data ir_ret ) )
            ? == rk 0 {
                ( string_push_str shims `  ret ` ) ( string_push_str shims ( string_data ir_ret ) )
                ...
                ( string_push_str shims ? == rk 4 ? == r_char 115 `zext` `sext` ( shim_conv_op rk ) )
                ...
                ? == ( vec_len [String] parts ) 3 {
                    ...
                    ? ( string_contains res ( string_data pat ) ) {
                        ...
                        ? == already_shimmed F {
                            ...
                            // match it exactly — see the note above `ir_decl_line`.
                            : i r_char ? > ( string_len ret ) 0 ( string_get ret 0 ) 0
                            : ~ String ir_ret ( shim_ret_from_letters r_char )
                            : ~ ( Vec String ) ir_pms ( shim_pms_from_letters pms )
                            : String decl ( ir_decl_line res ( string_data name ) )
                            ? > ( string_len decl ) 0 {
                                : String d_ret ( ir_decl_ret decl )
                                ? > ( string_len d_ret ) 0 { ( string_free ir_ret ) = ir_ret d_ret } { ( string_free d_ret ) }
                                : ( Vec String ) d_pms ( ir_decl_params decl )
                            ...
                            ( emit_libc_shim shims ( string_data name ) r_char pms ir_ret ir_pms )
                            ( string_free ir_ret )
                            ( vec_free_with [String] ir_pms \ String s → v { ( string_free s ) } )
                        } {}
                        ( string_free sname )
