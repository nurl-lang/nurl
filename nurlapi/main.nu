...
$ `packages/wasmbuilder/src/wasi_ir.nu`
$ `nurlapi/pptws.nu`
...
//
// The wasm32-wasi IR rewrite (rename `@main`, libc width-shims, POSIX
// stubs, env-import marking) lives in packages/wasmbuilder/src/wasi_ir.nu
// and is imported above — one rewriter for the local wasmbuilder CLI and
// this service. This file used to carry its own fork of that logic; the
// copies drifted (the fork predated the vararg `...` drop in
// `__wb_ir_decl_params`, so any program whose IR declares a vararg libc
// symbol — `open` via stdlib/core/io.nu, `fcntl`, `printf` — emitted the
// illegal `... %aN` in a define and failed to link). Fix it once, in the
// package.
...
@ __mcp_schema_api → Json {
    ...
    ( json_obj_set props `query`
    ( __mcp_prop `string` `Search every stdlib module's declarations instead: space- or comma-separated terms, ALL must occur (case-insensitive) in a declaration's signature + doc comment + module path. Keep one query to ONE concept (2–4 related terms, e.g. 'string lowercase') — a grab-bag like 'vec sort string split lowercase contains' does NOT get every term searched: when no declaration has all the terms, any term that exactly NAMES a stdlib module ('csv', 'csv.nu', or 'ext/csv.nu') returns that whole module's API surface and the reply answers ONLY those module-name terms — whole surfaces, never cut mid-module; surfaces that don't fit are listed by name for a follow-up module= call, and the remaining terms are listed at the end as NOT searched (issue each as its own query). Only when no term names a module are the terms re-run as an OR — each matched as a WHOLE word (bounded by a digit or any non-letter, so 'string' hits string_push_str but not substring) — with the best-covering declarations ranked, most terms covered first. Only when that finds nothing does the reply widen to example programs and registry packages. Ignored when 'module' or 'package' is set.` ) )
    ( json_obj_set schema `properties` props )
    ^ schema
}
