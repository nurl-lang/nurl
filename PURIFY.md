...
| 4  | File & process                    |  403 | OS-glue   | [~] | PARTIAL — Phase 7 batches 1+2 removed 10 file thunks (2026-05-23). §4 batch 3 (2026-05-24) replaced `nurl_realpath` (pure `& \`c\`` FFI to libc `realpath(path, NULL)` in `stdlib/std/path.nu`) and `nurl_write_file_safe` (`__write_file_pure` over libc fopen / fwrite / fclose in `stdlib/std/fs.nu`); −13 LOC C, −1 preamble declare. Win32 path canonicalisation now strictly POSIX-shaped via libmingwex's `realpath` (returns NULL for missing paths) — `path_canonical` callers prefer this. |
