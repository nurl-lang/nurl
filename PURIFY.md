...
| 4  | File & process                    |  403 | OS-glue   | [~] | PARTIAL — Phase 7 batches 1+2 removed 10 file thunks (2026-05-23). §4 batches 3+4 (2026-05-24) replaced `nurl_realpath`, `nurl_write_file_safe`, and `nurl_file_size` with pure-NURL `& \`c\`` FFI (`realpath` / `fopen` / `fwrite` / `fclose` / `fseek` / `ftell`); −16 LOC C, −2 preamble declares + `fseek`/`ftell` added for future migrations. `__file_size_pure` sidesteps `struct stat`'s platform-varying `st_size` offset by using fopen+fseek(SEEK_END)+ftell. Win32 path canonicalisation now strictly POSIX-shaped via libmingwex's `realpath`. |
...
| 13 | CLI tooling                       |  219 | OS-glue   | [~] | PARTIAL 2026-05-24 — batches 1 + 2 done: env / cwd / chdir + `read_all_stdin` are pure-NURL FFI now. Batch 1 (`getenv` / `setenv` / `unsetenv` / `getcwd` / `chdir` in `stdlib/ext/env.nu`) and batch 2 (`__read_all_stdin_pure` over `read(2)` in `stdlib/core/io.nu`) — −65 C LOC combined; 6 preamble declares + 6 `sym_def` entries gone. Remaining: `nurl_dir_list_*` (DIR* / FindFirstFile opaque iterator). |
