...
WEBDOCS_FACTS="$ROOT/webdocs/lib/release-facts.generated.ts"
WEBDOCS=0
[[ "${1:-}" == "--webdocs" ]] && WEBDOCS=1
[[ "$WEBDOCS" -eq 1 || -f "$HTML" ]] || { echo "gen-site-facts: $HTML not found" >&2; exit 1; }
...
if [[ "$WEBDOCS" -eq 1 ]]; then
    # Preserve the previous package count on a transient registry outage, just
    # as the old HTML injector does.
    if [[ -z "$registry_packages" && -f "$WEBDOCS_FACTS" ]]; then
        registry_packages="$(sed -n "s/^  \['\([^']*\)', 'registry packages'\],$/\1/p" "$WEBDOCS_FACTS" | head -1)"
    fi
    registry_packages="${registry_packages:-0}"
    cat > "$WEBDOCS_FACTS" <<EOF
...
EOF
    echo "release facts → $WEBDOCS_FACTS"
    exit 0
fi
