...
const WEBDOCS_OUTPUT = join(ROOT, "webdocs", "lib", "benchmark-stats.generated.ts");
...
if (WEBDOCS) {
    const webdocsRows = rows.map((bench) => {
        const values = COLUMNS.map((column) => bench.run_ms?.[column.key] ?? null);
        const labels = values.map(formatMs);
        const finite = values.filter((value) => typeof value === "number" && Number.isFinite(value));
        const bestLabel = finite.length > 0 ? formatMs(Math.min(...finite)) : null;
        const nurlWins = bestLabel !== null && labels[0] === bestLabel && !labels.slice(1).includes(bestLabel);
        return { name: bench.name, blurb: bench.blurb ?? "", values: labels, nurlWins };
    });
    const stats = {
        ...
        columns: COLUMNS.map(({ label, nurl = false }) => ({ label, nurl })),
        rows: webdocsRows,
    };
    writeFileSync(
        WEBDOCS_OUTPUT,
    ...
    console.log(`benchmark stats → ${WEBDOCS_OUTPUT}\n  ${rows.length} benchmarks × ${COLUMNS.length} languages, measured ${stats.generated}`);
    process.exit(0);
}
