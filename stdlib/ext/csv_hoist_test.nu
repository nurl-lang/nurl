...
//
// Phase 2c: hoist row-level FFI out of the per-row loop. Phase 2b
// still called 3 vec_reserve + 3 vec_data PER ROW; on a 1 M-row
// fixture that is 6 M FFI × ~30 ns ≈ 180 ms — a quarter of the load
// budget. Now we reserve all three index Vecs once at function entry
// (sized off `clen / 50` row estimate × 16 cells/row × 2 i64), cache
// data pointers + caps as NURL locals, and only re-FFI on a real
// grow (predicted-not-taken on typical CSV shapes). `wi` is hoisted
// too, replacing the per-row `vec_len . t flat_cells` re-read; the
// underlying len is committed once via `vec_set_len` at the end.
//
// Phase 2b background (still relevant): each `vec_push` calls
// `nurl_peek`/`nurl_poke` (external C, not inlined without LTO),
// costing ~30 ns per push × 16 M pushes = ~480 ms. By doing cell
// stores as raw `. fcp i` writes against a hoisted pointer we keep
// the byte+cell loops in pure native code — LLVM-O2 turns them into
// a tight scalar loop indistinguishable from a hand-written C parser.
//
...
@ __csv_a_parse_content * CSVTableA t CSVDialect dia → v {
    ...
    // Per-row safety margin in i64 units. 256 cells/row is far above
    // any realistic CSV; the safety check below trips before a row
    // can overrun this margin and grows + re-fetches the pointer.
    : i max_row_i64 * 256 2

    // One-shot reserves sized off file length. row_max_est tracks
    // upper-bound-ish row count (avg ≥50 B/row); est_cells_init = 16
    // covers ≤16-col CSVs without growing flat_cells. Wider rows
    // trigger doubling-grow inside the loop (cold path).
    : ~ i row_max_est / clen 50
    ? < row_max_est 16 { = row_max_est 16 } {}
    : i est_cells_init 16
    ( vec_reserve [i] . t flat_cells * row_max_est * est_cells_init 2 )
    ( vec_reserve [i] . t row_starts row_max_est )
    ( vec_reserve [i] . t row_lens row_max_est )

    // Hoist data pointers + caps. Mutated only when we have to grow;
    // stay constant on the steady-state hot path.
    : ~ * i fcp ( vec_data [i] . t flat_cells )
    : ~ * i rsp_w ( vec_data [i] . t row_starts )
    : ~ * i rlp_w ( vec_data [i] . t row_lens )
    : ~ i fc_cap ( vec_cap [i] . t flat_cells )
    : ~ i rs_cap ( vec_cap [i] . t row_starts )
    : ~ i rl_cap ( vec_cap [i] . t row_lens )

    : ~ i wi 0  // raw write index into flat_cells
    ...
    ~ < pos clen {
        ...
        // Per-row safety: grow + re-fetch any of the three buffers
        // whose cap is too tight for one more row. NURL-side
        // arithmetic only — zero FFI on the steady-state hot path.
        ? > + wi max_row_i64 fc_cap {
            ( vec_reserve [i] . t flat_cells + wi max_row_i64 )
            = fcp ( vec_data [i] . t flat_cells )
            = fc_cap ( vec_cap [i] . t flat_cells )
        } {}
        ? >= row_w rs_cap {
            ( vec_reserve [i] . t row_starts + row_w 1 )
            = rsp_w ( vec_data [i] . t row_starts )
            = rs_cap ( vec_cap [i] . t row_starts )
        } {}
        ? >= row_w rl_cap {
            ( vec_reserve [i] . t row_lens + row_w 1 )
            = rlp_w ( vec_data [i] . t row_lens )
            = rl_cap ( vec_cap [i] . t row_lens )
        } {}
        ...
        ? & is_phantom >= p clen {
            // drop phantom — rewind wi so its 2 i64 slot is reused
            // (or simply abandoned at end-of-parse; vec_set_len below
            // commits flat_cells at our final wi).
            = wi row_first_i64
            = pos p
        } {
            ? first_row {
                ...
                // Header cells were extracted into `t.headers`; rewind
                // wi so the body rows write starting at flat_cells[0].
                = wi row_first_i64
                = first_row F
            } {
    ...
    // Commit final lengths in one shot. Phase 2c hoist saves the
    // per-row vec_set_len on flat_cells too — was 1 FFI/row.
    : b _r0 ( vec_set_len [i] . t flat_cells wi )
