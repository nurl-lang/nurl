...
def rope(x, pos):
    # x: [n, heads, HD]; NEOX rotation over the first RD dims only
    inv = 1.0 / (BASE ** (np.arange(0, RD, 2) / RD))
    freqs = np.outer(pos, inv)                     # [n, RD/2]
    emb = np.concatenate([freqs, freqs], axis=-1)  # [n, RD]
    cos = np.cos(emb)[:, None, :]
    sin = np.sin(emb)[:, None, :]
    rot, rest = x[..., :RD], x[..., RD:]
...
n = len(ids)
# the shard stores the embedding as BF16 — mirror that rounding
...
for L in range(NL):
    ...
    q = rope(q, pos)
    k = rope(k, pos)
    # GQA: repeat kv heads; bidirectional attention over the window
    rep = NH // NKV
    ...
    if L < FIRST_DENSE:
        ...
        # router: fp32 sigmoid, bias for SELECTION only, group top-k
...
logits = x @ T["lm_head.weight"].astype(np.float64).T
for row in logits:
