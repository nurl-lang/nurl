...
@ test_oidc Router r → v {
    ...
    ( check ( jstr_eq . acs data `path` `data.items` ) `mcp: with its path` )
    ( check == ( jint_of . acs data `interval_minutes` ) 30 `mcp: and its schedule` )
    ( check ! ( string_contains . acs text `s3cret` ) `mcp: the header value is masked in the answer` )
    ...
    ( string_push_str one `"}` )
    : Call gone ( call r `source` ( string_data one ) AK )
