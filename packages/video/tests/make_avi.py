...
#   make_avi.py <out.avi> <fps> synth:<n>      n generated gradient frames
import io
import struct
import sys
...
for p in sys.argv[3:]:
    if p.startswith("synth:"):
        # Self-contained fixtures: n frames whose gradient shifts with the
        # index, so frame k is distinguishable from frame k+1 after a
        # decode. No files needed.
        w, h = 64, 48
        for k in range(int(p[6:])):
            im = Image.new("RGB", (w, h))
            im.putdata([((x * 4 + k * 16) % 256, (y * 5) % 256, (k * 40) % 256)
