...
//              [--page FILE]  (default: the template built into the binary)
//
...
// render the page template once with the static context
// The page template compiled into the binary (src/index_html_data.nu,
// generated by tools/embed_page.py) — an installed tool has no views/
// directory on disk. --page FILE overrides it.
@ yd_builtin_page → String {
    : ~ String out ( string_with_cap 24576 )
    : i nch ( index_html_chunks )
    : ~ i c 0
    ~ < c nch { ( string_push_str out ( index_html_chunk c ) ) = c + c 1 }
    ^ out
}
...
@ main → i {
    ...
    ( args_opt ap `page` 80 `FILE` `demo page template override (default: built into the binary)` )  // -P
    ...
    ? ( args_parse ap argv ) {
        ? ( args_present ap `help` ) {
            ...
            : ~ String page ( string_new )  // empty → built-in template
