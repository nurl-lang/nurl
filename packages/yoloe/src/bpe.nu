...
@ tokenizer_load s merges_path → Tokenizer {
    ...
    // specials — the scalar fields must be written HERE: a helper
    // receives the struct by value, so its map inserts stick (shared
    // handles) but a field write would land on the copy
...
// Build the tokenizer from the merge table compiled into the binary
// (src/clip_merges_data.nu, generated by tools/embed_merges.py). An
// installed tool has no package tree on disk, so this is the default;
// tokenizer_load stays for custom vocabularies via --merges.
@ tokenizer_load_builtin → Tokenizer {
    : ( Vec i ) order ( vec_new [i] )
    : ( Vec String ) byte_enc ( __build_byte_enc order )
    : Tokenizer tk @ Tokenizer {
        byte_enc
...
// Consume the whole merge table (one "a b" pair per line). Borrows buf.
@ __load_merges Tokenizer tk ( Vec u ) buf → v {
    : i n ( vec_len [u] buf )
    : ~ ( Vec u ) cur ( vec_new [u] )
    : ~ i rank 0
    : ~ i p 0
...
// by value: only the shared map/vec handles inside it can be mutated
