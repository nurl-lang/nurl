...
echo "[1/13] camera geometry vs the reference torch code"
...
echo "[2/13] frame preprocessing vs the reference load_fn pipeline"
...
echo "[3/13] position-grid resample vs torch bicubic+antialias"
...
echo "[4/13] 2-D rotary position embedding vs the reference"
...
echo "[5/13] patch embedding vs torch Conv2d"
...
echo "[6/13] full transformer block vs the reference Block"
...
echo "[7/13] the real 4.6 GB checkpoint (skipped when absent)"
if [ ! -f "$CKPT" ]; then
...
echo "[8/13] the block on the DEVICE (f32) vs the host reference"
...
echo "[9/13] 3-D rope vs the real WanRotaryPosEmbed"
...
echo "[10/13] the DINOv2 trunk on a real frame vs the real model"
...
echo "[11/13] the WHOLE aggregator on a real frame vs the real model"
...
echo "[12/13] two frames STREAMED through the KV cache"
...
echo "[13/13] a camera POSE, end to end, vs the real model"
