...
client would use. The page is embedded in the binary (`src/page_data.nu`,
regenerated by `tools/embed_page.py` from `views/index.html`), so an
installed tool needs nothing on disk.
