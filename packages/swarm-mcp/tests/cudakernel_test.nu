...
@ main → i {
    ...
    ( pb `reduce: out sized K+1 slots:       ` ( has prd `: i obytes * + kbins 1 16` ) )
    ( pb `reduce: overflow counter at slot K:` ( has prd `if (probe == K) atomicAdd((unsigned long long*)&ok[2*K], 1ULL);` ) )
    ( pb `reduce: control slot zeroed:        ` ( has prd `( nurl_poke zh * kbins 2 0 )` ) )
