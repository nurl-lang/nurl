    ...
    ~ & > ( vec_len [s] frames ) 0 == 0 . it halt {
        ...
        ~ < pc pend {
            ...
            ? < fuel 0 { ( __trap it `fuel exhausted` ) = . fr pos r0 = pc pend } {
                ...
                : i rc . pbase + r0 3
                = pc + pc 6
                // ── the dispatch, in measured frequency order ──
                ? | & >= op 124 <= op 126 & >= op 131 <= op 138 {  // i64 arithmetic / bitwise
                    = . rbase ra ( __rnum64 op . rbase rb . rbase rc )
                } {
                    ? >= op 300 {  // the register/control forms, one dense table
                        ...
                        ? | & >= op 106 <= op 108 & >= op 113 <= op 120 {  // i32 arithmetic / bitwise
                            = . rbase ra ( __rnum32 op . rbase rb . rbase rc )
                        } {
                            ? | & >= op 81 <= op 90 & >= op 70 <= op 79 {  // integer compares
                                = . rbase ra ( __rcmp op . rbase rb . rbase rc )
                            } {
                                ? & >= op 40 <= op 53 {  // loads
                                    = . rbase ra ( __rload it op + & . rbase rb 4294967295 rc )
                                    ? != 0 . it halt { = . fr pos r0 = pc pend } {}
                                } {
                                    ? | & >= op 91 <= op 102 & >= op 139 <= op 166 {  // float arithmetic, unary and compares
                                        = . rbase ra ( __farith op . rbase rb . rbase rc )
                                    } {
                                        ? & >= op 167 <= op 191 {
                                            // i32.wrap_i64 and i64.extend_i32_s/u are how a 64-bit guest
                                            // language addresses 32-bit linear memory, so they are as common
                                            // here as a `local.get` and they are one instruction; reinterpret
                                            // is none at all. Only the truncations are worth a call.
                                            ? | == op 167 == op 172 { = . rbase ra ( __w32 . rbase rb ) } {
                                                ? == op 173 { = . rbase ra & . rbase rb 4294967295 } {
                                                    ? & >= op 188 <= op 191 { = . rbase ra . rbase rb } {
                                                        = . rbase ra ( __convert it op . rbase rb )
                                                        ? != 0 . it halt { = . fr pos r0 = pc pend } {}
                                                    } } }
                                        } {
                                            ? & >= op 54 <= op 62 {  // stores
                                                ( __rstore it op + & . rbase ra 4294967295 rc . rbase rb )
                                                ? != 0 . it halt { = . fr pos r0 = pc pend } {}
                                            } {
                                                ? | == op 69 == op 80 { = . rbase ra ? == . rbase rb 0 1 0 } {  // eqz
                                                    ? | & >= op 127 <= op 130 & >= op 109 <= op 112 {  // div / rem
                                                        = . rbase ra ( __rdiv it op . rbase rb . rbase rc )
                                                        ? != 0 . it halt { = . fr pos r0 = pc pend } {}
                                                    } {
                                                        // clz / ctz / popcnt and the sign extensions
