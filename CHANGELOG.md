...
- **`packages/anomaly` 0.6.0 and 0.7.0 — the model's metadata became
  editable, and then the editor stopped being written by hand.**

  0.6.0 opened `PUT /models/dynamic/<model>/metadata`: the editable half
  of a model's metadata — the retrain schedule and the per-version
  configs — as a patch in which every field is optional, so a checkbox
  can send one field. An unknown version name adds a version with the
  defaults filling the gaps; `replace_versions` deletes the ones the body
  omits. Values are clamped into a range the trainer can honour rather
  than rejected, so a hand-written edit cannot produce a version that
  fails to train. The *learned* half — column kinds, category
  vocabularies, feature order, scaler — is still never accepted from a
  client: it is refitted at every train, and a hand-written copy would
  silently desync every forest. The same release made the autoencoder
  visible (it had been trainable since 0.4.0 and its verdict had ridden
  along in every detect, with nothing in the dashboard saying so) and made
  disabling its version actually mute it.

  0.7.0 turned the ring size into metadata too. `max_data_points` was
  `MAX_DATA_POINTS = 150000` compiled in, so a model that should keep a
  thousand points kept a hundred and fifty thousand. It is now per-model,
  persisted, and patchable — positive and at least `min_data_points`,
  since a ring smaller than the warm-up threshold could never train the
  model at all — and lowering it below the current fill evicts the oldest
  points and rewrites the log *before the call returns*, rather than
  converging on the new cap one ingest at a time.

  And that field is why 0.7.0 also rewrote the editor. The dashboard's
  Advanced box hard-coded the editable half as `{schedule, versions}` —
  its own copy of a list that lives in `model_apply_meta_patch` — so the
  new field was patchable through the API and invisible in the one editor
  that exists to reach it. The list is now published: every metadata
  response carries `editable_fields`, and the box has two views over
  exactly those keys — a **form generated by walking the metadata's own
  shape** (one typed input per leaf, every version and every one of its
  fields, including the geometry and forest sizes no hand-written form
  ever showed) and the raw JSON. Neither view names a field, so the next
  editable key appears on its own.
