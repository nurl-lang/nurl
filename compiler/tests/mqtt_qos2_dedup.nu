...
// even when the broker retransmits it (DUP). _mqtt_qos2_seen_or_add
// records a packet id and reports whether it was already pending;
// _mqtt_qos2_forget drops it on clean handshake completion; the set is
...
@ main → i {
    ...
    ( nurl_print `first=` ) ( nurl_print ( yn ( _mqtt_qos2_seen_or_add s 5 ) ) ) ( nurl_print `\n` )
    // Same pid again → duplicate.
    ( nurl_print `dup=` ) ( nurl_print ( yn ( _mqtt_qos2_seen_or_add s 5 ) ) ) ( nurl_print `\n` )
    // A different pid → not a duplicate.
    ( nurl_print `other=` ) ( nurl_print ( yn ( _mqtt_qos2_seen_or_add s 6 ) ) ) ( nurl_print `\n` )
    ...
    ( _mqtt_qos2_forget s 5 )
    ( nurl_print `forgot_len=` ) ( nurl_print ( nurl_str_int ( vec_len [i] s ) ) ) ( nurl_print `\n` )
    ( nurl_print `readd=` ) ( nurl_print ( yn ( _mqtt_qos2_seen_or_add s 5 ) ) ) ( nurl_print `\n` )
    ...
    : ( Vec i ) s2 ( vec_new [i] )
    : ~ i j 1
    ~ <= j 256 { ( _mqtt_qos2_seen_or_add s2 j ) = j + j 1 }
    ( nurl_print `cap_len=` ) ( nurl_print ( nurl_str_int ( vec_len [i] s2 ) ) ) ( nurl_print `\n` )
    ( nurl_print `has1=` ) ( nurl_print ( yn ( _mqtt_qos2_seen_or_add s2 1 ) ) ) ( nurl_print `\n` )
    ( _mqtt_qos2_seen_or_add s2 257 )
    ( nurl_print `after257_len=` ) ( nurl_print ( nurl_str_int ( vec_len [i] s2 ) ) ) ( nurl_print `\n` )
    ( nurl_print `has1_after=` ) ( nurl_print ( yn ( _mqtt_qos2_seen_or_add s2 1 ) ) ) ( nurl_print `\n` )
    ( vec_free [i] s )
    ( vec_free [i] s2 )
    ^ 0
}
