...
@ run b reverse → String {
    ...
    : !( Vec LockPkg ) ResolveErr result ( resolve_registry roots `https://unused.test/` \ s reg s name → String { ^ ( fetch reg name ) } )
    ( vec_free_with [Dep] roots \ Dep d → v { ( dep_free d ) } )
    ?? result {
        F e → { ( nurl_println ( resolve_err_name e ) ) ^ ( string_new ) }
        T locked → {
