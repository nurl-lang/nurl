...
[ -n "$SRC" ] || {
    echo "usage: build_unikernel_arm64.sh prog.nu [-o out.elf] [--fs dir] [--disk] [--out-dir dir]" >&2
    exit 2
}
[ -x "$ROOT/build/nurlc" ] || {
...
cache_inputs() {
    printf '%s\n' \
        ...
        "$BOOT"/nosys.c "$BOOT"/vfs.c "$BOOT"/fdt.c "$BOOT"/cmdenv.c \
...
$ZIG cc -target $TARGET -nostdlib -static -Wl,-T,"$BOOT/link_arm64.ld" \
    ...
    "$CACHE/boot_fdt.o" "$CACHE/boot_cmdenv.o" "$CACHE/cuda_stubs.o" "$CACHE/nvrtc_stubs.o" \
