...
Primer **v3** keeps the focused ~1.3k-token reference and adds **one prominent, general rule**: stdlib functions (above all `nurl_str_*`) need their `$` import — stated at the very top and shown in the canonical worked example. It is **1,604 tokens** (a ~270-token addition, not the 14k grammar dump that failed — see [`RESULTS_old.md`](RESULTS_old.md) and `primers/nurl_old.md`). This run also adds a **fourth model: `mercury-2`, Inception Labs' diffusion LLM** (driven by `generate_inception.py`).

**The targeted cue worked.** Import-omission — the dominant held-out failure in every prior run — collapsed across all four models: **sonnet 20→0, opus →0, haiku 6→1, mercury 0**.
...
| `mercury-2` (diffusion) | **15%** | **15%** | 100% | 100% |
...
- **mercury** is dominated by the same grouping-paren habit (11 of ~17 failures, across `|` `.` `&` `>=` `==` `*`).

**The diffusion model, sharply.** `mercury-2` writes **Python and Rust at 100%** — a capable code model — yet reaches only **15%** on NURL, almost entirely because it cannot stop wrapping operator expressions in parentheses. It is the cleanest demonstration that the gap is out-of-distribution *habit*, not raw capability.

**Honest bottom line.** A focused, targeted primer cue reliably fixes the *specific* failure it targets (v3 killed import-omission everywhere, the diffusion model included). But first-try accuracy on a language with **zero training presence** is gated by a *sequence* of in-distribution habits — imports, then grouping-parentheses, then mutability — each surfacing as the last is fixed. **No model reaches Python/Rust parity**, and a fully out-of-distribution model sits far below it. The defensible claim holds: a regular grammar is learnable from a focused page and individual failure modes are fixable with targeted cues — not that NURL matches mainstream languages for first-try generation.
