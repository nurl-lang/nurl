...
def main() -> int:
    ...
    emit()
    emit("Generated by `bench/genacc/score.py`. Figures are **first-pass** — the "
         "model's first attempt, no human edits — except in conditions labelled "
         "as a repair round: a `…repair` directory (made by `repair.py`) adds "
         "ONE round of compiler-error feedback, in which the model sees only "
         "its own program and the compiler's stderr, never the expected "
         "output. *compile* = built with zero changes; *correct* = printed "
    ...
    for d in args.dirs:
        ...
        if "repair" in d:
            label = "first-pass + one compiler-feedback repair round (repair.py)"
        if not runs:
            emit(f"## `{d}`")
            emit()
    ...
    emit()
    emit("- **First-pass is the strictest setting; the repair condition is the "
         "realistic one.** A first-pass row allows no retries, no human edits, "
         "no feedback. A `…repair` row replays what a real agent does: one "
         "round of compiler-diagnostic feedback (stderr only — the expected "
         "output is never shown), applied identically to every language.")
