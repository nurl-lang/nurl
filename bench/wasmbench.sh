...
emit_md() {
    ...
    printf 'Read the wasm columns knowing what each toolchain ships by DEFAULT,\n'
    printf 'because that is what this table measures and the defaults differ.\n'
    printf 'Since wasmbuilder 0.3.0 the NURL column is stripped of debug info, which\n'
    printf 'is what `nurl.sh` has always done natively — the native columns beside it\n'
    printf 'carry no DWARF either, in any of the three languages. `zig cc` and\n'
    printf '`rustc` keep theirs for wasm, and it dominates them: an unstripped NURL\n'
    printf 'module was 1,126,353 bytes for the empty program, of which 3,070 were\n'
    printf 'code. So the NURL wasm column is roughly code, and the C and Rust wasm\n'
    printf 'columns are roughly debug info. Strip all three and they land within a\n'
    printf 'factor of a few; that comparison is not run here because a benchmark of\n'
    printf 'shipped artefacts should report what each toolchain actually ships.\n'
    printf 'Module-load time is unaffected either way — a runtime skips custom\n'
    printf 'sections — so nothing in sections 1-3 moves with this.\n\n'
