...
// Execute one CPU step with sub-instruction-accurate clocking (see the
// CLI core for the rationale): the opcode-fetch M-cycle is clocked before
// the instruction body, so reads of LY/TIMA/STAT observe the right cycle.
@ cpu_advance → i {
    : ~ i used 4
    ? == g_halt 0 {
        ( clock 4 )
